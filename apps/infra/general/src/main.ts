...
import { AppRunnerStack, EnvironmentConfig } from './app-runner-stack';
...
new AppRunnerStack(app, `AppRunner${capitalize(environment)}Stack`, config, {
  stackName: `app-runner-${environment}`,
  env: {
    account: process.env.CDK_DEFAULT_ACCOUNT,
    region: process.env.CDK_DEFAULT_REGION || 'ca-central-1',
  },
  description: `App Runner infrastructure for ${environment} environment`,
});
